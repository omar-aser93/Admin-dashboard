...
const Dashboard = () => {
  ...
  const { data, isLoading } = useGetDashboardQuery();            //get Dashboard data response from (Redux-TK query)
  const isNonMediumScreens = useMediaQuery("(min-width: 1200px)");   //boolean checks screen size using MUI (useMediaQuery)   
  ...
  return (
    <Box m="1.5rem 2.5rem" pb='20px' >
      <Box sx={{display: "flex", justifyContent: "space-between", alignItems: "center"}}>
        {/* Header */}
        <Box>
         <Typography variant="h2" color={theme.palette.secondary[100]} fontWeight="bold" > DASHBOARD </Typography>
         <Typography variant="h5" color={theme.palette.secondary[300]} sx={{ mt: "5px" }}> Welcome to your dashboard </Typography>
        </Box>
        {/* Button */}
        <Box>
        ...
        <Box gridColumn="span 8" gridRow="span 3"
             sx={{ "& .MuiDataGrid-root": { border: "none", borderRadius: "5rem" }, "& .MuiDataGrid-cell": { borderBottom: "none" },
