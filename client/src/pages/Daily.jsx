...
const Daily = () => {
  ...
  const { data } = useGetSalesQuery();                                    //get Sales response from (Redux-TK query)  
  const [startDate, setStartDate] = useState(new Date("2021-02-01"));     //state to store the start date (feb as default)
  const [endDate, setEndDate] = useState(new Date("2021-03-01"));         //state to store the end date   
  ...
  return (
    <Box m="1.5rem 2.5rem" >
      {/* Header */}
      <Box>
       <Typography variant="h2" color={theme.palette.secondary[100]} fontWeight="bold" > DAILY SALES </Typography>
       <Typography variant="h5" color={theme.palette.secondary[300]} sx={{ mt: "5px" }}> Chart of daily sales </Typography>
      </Box>
      ...
      <Box height="75vh">
        ...
        {data ? (
          <ResponsiveLine data={formattedData}
            theme={{ axis: { domain: { line: { stroke: theme.palette.secondary[200] } },
            ...
            colors={{ datum: "color" }} margin={{ top: 50, right: 50, bottom: 70, left: 60 }} xScale={{ type: "point" }}
            yScale={{ type: "linear", min: "auto", max: "auto", stacked: false, reverse: false }} yFormat=" >-.2f"
            curve="catmullRom" axisTop={null}  axisRight={null}
            ...
            pointBorderWidth={2}  pointBorderColor={{ from: "serieColor" }} pointLabelYOffset={-12} useMesh={true}
            legends={[ { anchor: "top-right", direction: "column", justify: false, translateX: 50, translateY: 0, itemsSpacing: 0,
                itemDirection: "left-to-right", itemWidth: 80,  itemHeight: 20, itemOpacity: 0.75, symbolSize: 12, symbolShape: "circle", 
                symbolBorderColor: "rgba(0, 0, 0, .5)", effects: [{ on: "hover", style: { itemBackground: "rgba(0, 0, 0, .03)", itemOpacity: 1 }} ]
              },
            ]} />
        ) : ( <>Loading...</>  )}
      </Box>
