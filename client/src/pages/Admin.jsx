...
const Admin = () => {
  ...
  const { data , isLoading} = useGetAdminsQuery();         //get Admins data response from (Redux-TK query) 
  //Array to store columns header data for the MUI Grid
  const columns = [
    { field: "_id", headerName: "ID", flex: 1 }, { field: "name", headerName: "Name", flex: 0.5 }, { field: "email", headerName: "Email", flex: 1 },
  ...
  return (
    <Box m="1.5rem 2.5rem" pb='15px'>
      {/* Header */}
      <Box>
       <Typography variant="h2" color={theme.palette.secondary[100]} fontWeight="bold" > ADMINS </Typography>
       <Typography variant="h5" color={theme.palette.secondary[300]} sx={{ mt: "5px" }}> Managing admins and list of admins </Typography>
      </Box>
      ...
      <Box mt="40px" height="75vh" sx={{ "& .MuiDataGrid-root": { border: "none" }, "& .MuiDataGrid-cell": { borderBottom: "none" },
        ...
        <DataGrid loading={isLoading || !data}  getRowId={(row) => row._id}  rows={data || []} columns={columns} components={{ ColumnMenu: CustomColumnMenu }} />
